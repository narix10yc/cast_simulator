...
pub fn measure_cpu_profile(spec: &CPUKernelGenSpec) -> anyhow::Result<HardwareProfile> {
    ...
    for &s in seed_opcounts {
        sweep.push(probe_opcount(s, spec, N_QUBITS_SV, n_threads, N_WARMUP, N_ITERS)?);
    }
...
fn probe_opcount(
    ...
    let n = n_qubits_sv.max(gate.n_qubits() + get_simd_s(spec.simd_width, spec.precision) + 1);
...
fn make_sparse_real_gate(k: usize, s: usize) -> QuantumGate {
    ...
    for row in 0..n {
        for t in 0..s {
            ...
            m.set(row, col, Complex::new(if rng.gen_bool(0.5) { v } else { -v }, 0.0));
        }
    }
...
fn fit_knee(sweep: &[(f64, f64, f64)]) -> (f64, f64) {
    ...
    let knee = if c_slope > 0.0 { peak_gflops / c_slope } else { f64::INFINITY };
    (knee, c_slope)
}
