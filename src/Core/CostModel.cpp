...
//   CUresult rc = cuFuncGetAttribute(&numRegs, CU_FUNC_ATTRIBUTE_NUM_REGS,
//   func); if (rc != CUDA_SUCCESS) {
...
//   // Device properties (unchanged for the whole run, so probably should
//   cache) static cudaDeviceProp props; static bool propsInit = false; if
//   (!propsInit) {
...
//       std::max(1u, std::min({maxBlocksThreads, maxBlocksRegs,
//       maxBlocksSmem}));
...
//   if (!controls.empty()) {
//     score *=
//         0.6 * pow(0.9, controls.size()); // Base penalty + diminishing
//         returns
//   }
...
// // Not included: Warp Efficiency Metrics, Shared Memory Bank Conflict
// Detection,
...
//     const legacy::QuantumGate& gate, Precision precision, int blockSize)
//     const {
...
//   double blkScale = std::pow(double(anchor->blockSize) / currentBlockSize,
//   0.5); double occScale = occLive / std::max(0.05, anchor->occupancy); double
//   coalScale =
...
// void CUDAPerformanceCache::runExperiments(const CUDAKernelGenConfig&
// gpuConfig,
//                                           int nQubits,
//                                           int blockSize,
//                                           int nRuns,
...
//           kernelMgr.genCUDAGate(gpuConfig, gate, "gate_" +
//           std::to_string(i++));
//       },
...
//     std::cerr << ": " << memSpeed << " GiB/s, " << "occupancy=" << occupancy
