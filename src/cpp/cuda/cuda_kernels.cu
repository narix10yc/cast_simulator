...
template <> struct Vec<float>  { using type = float4;  static constexpr uint64_t N = 4; };
template <> struct Vec<double> { using type = double2; static constexpr uint64_t N = 2; };
...
static void choose_launch_config(uint64_t work_items, int block_size,
    ...
    unsigned int needed = static_cast<unsigned int>(
        (work_items + block - 1) / block);
...
static int report_cuda_error(const char *op, cudaError_t err,
                             char *err_buf, size_t err_buf_len) {
    if (err_buf && err_buf_len > 0)
        snprintf(err_buf, err_buf_len, "%s failed: %s", op,
...
static int ensure_norm_scratch(unsigned int grid,
                               char *err_buf, size_t err_buf_len) {
    if (g_norm_block_sums && g_norm_grid >= grid)
        return 0; // already large enough
    if (g_norm_block_sums) cudaFree(g_norm_block_sums);
    if (g_norm_result)     cudaFree(g_norm_result);
    CUDA_CHECK(cudaMalloc(&g_norm_block_sums, grid * sizeof(double)),
               "cudaMalloc (norm block_sums)");
    CUDA_CHECK(cudaMalloc(&g_norm_result, sizeof(double)),
               "cudaMalloc (norm result)");
    g_norm_grid = grid;
    return 0;
}
...
template <typename T>
__global__ void norm_sq_pass1(const T * __restrict__ data,
    ...
    for (uint64_t i = gid; i < n_vec; i += stride) {
        VecT v = vec_data[i];
        if constexpr (VN == 4) {
            sum += double(v.x) * double(v.x) + double(v.y) * double(v.y)
...
__global__ void norm_sq_pass2(const double * __restrict__ block_sums,
...
template <typename T>
__global__ void scale_kernel(T * __restrict__ data, T scale, uint64_t n) {
    using VecT = typename Vec<T>::type;
    constexpr uint64_t VN = Vec<T>::N;
...
template <typename T>
static int norm_squared_impl(uint64_t dptr, size_t n_elements,
                             double *out_norm_sq,
                             char *err_buf, size_t err_buf_len) {
    ...
    CUDA_CHECK(cudaMemset(g_norm_result, 0, sizeof(double)),
               "cudaMemset (norm result)");

    norm_sq_pass1<T><<<grid, block>>>(
        reinterpret_cast<const T *>(dptr), g_norm_block_sums,
        static_cast<uint64_t>(n_elements));

    unsigned int pass2_threads =
        (static_cast<unsigned int>(grid) + 31u) & ~31u;
    if (pass2_threads > 256) pass2_threads = 256;
    norm_sq_pass2<<<1, pass2_threads>>>(g_norm_block_sums, g_norm_result, grid);

    CUDA_CHECK(cudaMemcpy(out_norm_sq, g_norm_result, sizeof(double),
...
template <typename T>
static int scale_impl(uint64_t dptr, size_t n_elements, double scale,
                      char *err_buf, size_t err_buf_len) {
    constexpr int BLOCK = 256;
    unsigned int grid, block;
    choose_launch_config(n_elements / Vec<T>::N, BLOCK, grid, block);

    scale_kernel<T><<<grid, block>>>(
        reinterpret_cast<T *>(dptr), static_cast<T>(scale),
        static_cast<uint64_t>(n_elements));
...
int cast_cuda_scale(uint64_t dptr, size_t n_elements, uint8_t precision,
                    ...
                    char *err_buf, size_t err_buf_len) {
    return (precision == 0)
        ? scale_impl<float>(dptr, n_elements, scale, err_buf, err_buf_len)
