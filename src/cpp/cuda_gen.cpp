...
static std::vector<IRMatDataCUDA>
getMatDataCUDA(IRBuilder<> &B,
  ...
  for (unsigned r = 0; r < K; ++r) {
    for (unsigned c = 0; c < K; ++c) {
      ...
      case SK_Zero:     data[idx].reVal = zeroVal;     break;
      case SK_One:      data[idx].reVal = oneVal;      break;
      case SK_MinusOne: data[idx].reVal = minusOneVal; break;
      case SK_ImmValue:
        data[idx].reVal = ConstantFP::get(scalarTy, fp32 ? static_cast<double>(static_cast<float>(re)) : re);
        break;
      default: break;
      }

      switch (data[idx].imKind) {
      case SK_Zero:     data[idx].imVal = zeroVal;     break;
      case SK_One:      data[idx].imVal = oneVal;      break;
      case SK_MinusOne: data[idx].imVal = minusOneVal; break;
      case SK_ImmValue:
        data[idx].imVal = ConstantFP::get(scalarTy, fp32 ? static_cast<double>(static_cast<float>(im)) : im);
        break;
      default: break;
      }
    }
...
static Function *getFunctionDeclarationCUDA(IRBuilder<> &B,
                                             Module &M,
  ...
  args.pSvArg  = func->getArg(0); args.pSvArg->setName("p.sv");
  args.pMatArg = func->getArg(1); args.pMatArg->setName("p.mat");
  args.pCombos = func->getArg(2); args.pCombos->setName("p.combos");
...
static Value *buildOffset(IRBuilder<> &B,
                           Value *counterV,
...
cast_cuda_generate_kernel_ir(const cast_cuda_kernel_gen_spec_t &spec,
  ...
  if (spec.precision != CAST_CUDA_PRECISION_F32 &&
      spec.precision != CAST_CUDA_PRECISION_F64)
    return llvm::createStringError("spec.precision must be F32 or F64");

  auto &ctx = module.getContext();
  IRBuilder<> B(ctx);

  Type *scalarTy = (spec.precision == CAST_CUDA_PRECISION_F32)
                       ? B.getFloatTy()
                       : B.getDoubleTy();
