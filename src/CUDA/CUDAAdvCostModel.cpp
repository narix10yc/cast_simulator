...
static inline double bytesState(int nQubits, Precision p) {
  ...
  const uint64_t elemBytes = (p == Precision::F32 ? 8ULL : 16ULL);
...
static AccessPattern classifyPatternVec(const std::vector<int>& qs) {
  ...
  const auto qmin = *std::ranges::min_element(qs);
  if (qmin <= 3)
    return AccessPattern::Contiguous;
  if (qmin <= 8)
...
static double timeKernelSeconds(const std::function<void()>& launch,
  ...
  return static_cast<double>(bestMs) * 1e-3;
}
...
static CUfunction buildNullKernelPTX(CUmodule& modOut) {
  ...
  unsigned int lvl = 0;
  void* opts[] = {&lvl};
  if (cuModuleLoadDataEx(&mod, ptx.c_str(), 1, &opt, opts) != CUDA_SUCCESS)
    return nullptr;
...
// Return the result in microseconds (us)
...
void CUDAAdvPerfCache::runPreliminaryExperiments(const CUDAKernelGenConfig& cfg,
  ...
  for (int k = 1; k <= 5; ++k) {
    for (int r = 0; r < 2; ++r) {
      ...
      const std::string name =
          "pre_k" + std::to_string(k) + "_" + std::to_string(r);
  ...
  if ((int)weights.size() < 5)
    weights.resize(5, 0);
  weights[0] = 100; // 1q base
  for (int k = 2; k <= 5; ++k) {
    constexpr double ratio = 1.1;
    weights[k - 1] =
        int(double(weights[k - 2]) *
            ((ratio - 1.0) * (memSpd[k - 2] / std::max(1e-9, memSpd[k - 1])) +
             2 - ratio));
  }
...
void CUDAAdvPerfCache::runExperiments(const CUDAKernelGenConfig& cfg,
  ...
  auto addRandomGate = [&](float erasureProb) {
    ...
    for (int i = 0; i < (int)nQubitsWeights.size(); ++i) {
      ...
      if (r < acc) {
        std::vector<int> tgt;
        utils::sampleNoReplacement(nQubits, i + 1, tgt);
  ...
  const int initial = gates.size();
  for (int run = 0; run < nRuns - initial; ++run)
    addRandomGate(/*eraseProb*/ 0.0f);
  ...
  for (const auto& uptr : km.getAllStandaloneKernels()) {
    ...
    const auto kq = k.gate->nQubits();
    const auto oc = k.opCount;
    const auto blk = (cfg.blockSize > 0) ? cfg.blockSize : 256;

    // Time
    const double time_s = timeKernelSeconds(
        [&]() { km.launchCUDAKernel(sv.dData(), nQubits, k, blk); }, repeats);
    ...
    double occ = occupancyFromDriver(
        dev, k.kernelFunction(), blk_for_occ, k.sharedMemUsage());
    ...
    Item item{.nQubitsGate = kq,
              ...
              .blockY = 1,
              .blockZ = 1,
              .regsPerThread = static_cast<int>(k.registerUsage()),
              .smemPerBlock = smemStatic,
              .occupancy = occ,
              .time_s = time_s,
              .bytes = bytes,
              .flops = flops,
              .Bach_GBs = (bytes / time_s) * 1e-9,
              .Pach_GFLOPs = (flops / time_s) * 1e-9};
  ...
  if (verbose) {
    ...
    if (t_us > 0)
      std::cerr << "[bench] measured launch latency ~ " << t_us << " us\n";
  }
}
...
void CUDAAdvCostModel::fitFromCache() {
  ...
  B_peak_GBs_.clear();
  F_peak_GFLOPs_.clear();
  for (auto& kv : B_buckets)
    B_peak_GBs_[kv.first] = percentile(kv.second, 0.95);
  for (auto& kv : F_buckets)
    F_peak_GFLOPs_[kv.first] = percentile(kv.second, 0.95);

  // f_coal: median ratio vs contiguous median
  double contigMed = 0.0;
  if (auto it = B_by_pattern.find(AccessPattern::Contiguous);
      it != B_by_pattern.end() && !it->second.empty())
  ...
  for (AccessPattern p :
       {AccessPattern::Contiguous, AccessPattern::Semi, AccessPattern::Strided})
    if (!f_coal_.count(p))
      f_coal_[p] = (p == AccessPattern::Contiguous ? 1.0 : 0.5);
  ...
  for (int b = 0; b <= 100; b += 5) {
    ...
    if (haveB) {
      ...
      if (p90 > 0)
        gB_lut_.push_back({b / 100.0, std::clamp(med / p90, 0.1, 1.0)});
    }
    if (haveF) {
      const double med = percentile(itF->second, 0.50);
      const double p90 = percentile(itF->second, 0.90);
      if (p90 > 0)
        gF_lut_.push_back({b / 100.0, std::clamp(med / p90, 0.1, 1.0)});
    }
  }
  if (gB_lut_.empty())
    gB_lut_ = {{0.0, 0.4}, {0.5, 0.9}, {1.0, 1.0}};
  if (gF_lut_.empty())
    gF_lut_ = {{0.0, 0.4}, {0.5, 0.9}, {1.0, 1.0}};

  // launch latency: prefer direct Driver measurement, otherwise fallback to 5th
  // pct of very small kernels
  ...
  if (t_us > 0.0) {
    ...
    if (!tiny.empty())
      t_launch_s_ = std::max(0.0, percentile(tiny, 0.05));
  }
}
...
double CUDAAdvCostModel::computeTime(const QuantumGate* gate,
  ...
  const double Bpeak = [this, &q]() {
    auto it = B_peak_GBs_.find(q.precision);
  ...
  const double Fpeak = [this, &q]() {
    auto it = F_peak_GFLOPs_.find(q.precision);
  ...
  // work
  const int nSweeps = 1;
  const double bytes = estimateBytes(
      q.nQubits, q.precision, q.coverageFraction, nSweeps, q.bytesFixup);
...
double CUDAAdvCostModel::computeGiBTime(const QuantumGate* gate) const {
  ...
  // Compute-path per GiB for this shape (rarely dominant for SV simulators, but
  // keep it)
  ...
  const double flops_per_byte =
      (opCount * FLOPS_PER_OP) / (2.0 * elemBytes * std::ldexp(1.0, k));
  const double Fpeak = [this, &q]() {
    auto it = F_peak_GFLOPs_.find(q.precision);
    return (it == F_peak_GFLOPs_.end() ? 1.0 : it->second);
...
std::ostream& CUDAAdvCostModel::displayInfo(std::ostream& os,
  ...
  for (const auto& kv : B_peak_GBs_) {
    os << "    prec=f" << int(kv.first) << " => " << kv.second << "\n";
  }
  os << "  F_peak (GFLOP/s):\n";
  ...
  if (verbose) {
    os << "  f_coal(pattern): ";
    for (const auto& kv : f_coal_)
      os << int(kv.first) << "->" << kv.second << "  ";
    os << "\n";
  }
  return os;
}
