...
__global__ void permute_lowbits_kernel(const Scalar* __restrict__ src,
                                       ...
                                       uint64_t maskLow, int k, int nSys)
{
  ...
  for (uint64_t j = blockIdx.x * blockDim.x + threadIdx.x;
       j < N; j += uint64_t(gridDim.x) * blockDim.x) {
    const uint64_t lo = (k ? (j & ((1ull << k) - 1ull)) : 0ull);
    const uint64_t hi = (k ? (j >> k) : j);
    const uint64_t i  = deposit_by_mask(lo, maskLow) | deposit_by_mask(hi, maskHigh);
    dst2[j] = src2[i];
  }
}
                                         ...
                                         cudaStream_t stream)
{
  ...
  dim3 block(256), grid((unsigned)std::min<uint64_t>((N + block.x - 1)/block.x, 65535));
  permute_lowbits_kernel<float, V2><<<grid, block, 0, stream>>>(src, dst, maskLow, k, nSys);
}
                                         ...
                                         cudaStream_t stream)
{
  ...
  const uint64_t N = 1ull << nSys;
  dim3 block(256), grid((unsigned)std::min<uint64_t>((N + block.x - 1)/block.x, 65535));
  permute_lowbits_kernel<double, V2><<<grid, block, 0, stream>>>(src, dst, maskLow, k, nSys);
}
