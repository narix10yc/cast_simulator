...
std::vector<IRMatDataCUDA> getMatDataCUDA(
  ...
  if (config.forceDenseKernel) {
      ...
      // not forceDenseKernel: determine reKind and imKind based on the matrix elements
  ...
  for (unsigned i = 0; i < KK; ++i) {
    switch (data[i].reKind) {
      case SK_Runtime: break;
      case SK_Zero: data[i].reVal = zeroVal; break;
      case SK_One: data[i].reVal = oneVal; break;
      case SK_MinusOne: data[i].reVal = minusOneVal; break;
      case SK_ImmValue: {
      ...
      switch (data[i].imKind) {
          case SK_Runtime: break;
          case SK_Zero: data[i].imVal = zeroVal; break;
          case SK_One: data[i].imVal = oneVal; break;
          case SK_MinusOne: data[i].imVal = minusOneVal; break;
          case SK_ImmValue: {
              auto im = matrix.imData()[i];
              data[i].imVal = ConstantFP::get(
                  B.getContext(),
                  (config.precision == Precision::F32)
...
Function *getFunctionDeclarationCUDA(
    ...
    //                         ? FunctionType::get(
    //                               B.getVoidTy(), {B.getPtrTy(), B.getPtrTy()}, false)
    //                         : FunctionType::get(
    //                               B.getVoidTy(), {B.getPtrTy()}, false);
    SmallVector<Type*, 3> params;
    params.push_back(B.getPtrTy()); // p.sv
    if (needsMatArg) params.push_back(B.getPtrTy()); // p.mat
...
Value *getGlobalTidCUDA(IRBuilder<> &B) {
    ...
    globalTidV = B.CreateIntCast(
        globalTidV, B.getInt64Ty(), true, "global.tid"
    );
    return globalTidV;
}
 ...
 */
GlobalVariable *createGlobalMatrixArray_NoUnroll(
  Module &M,
  Type *scalarTy,
...
void genMatrixVectorMultiply(
  ...
  GlobalVariable *gMatImmediate =
      createGlobalMatrixArray_NoUnroll(M, scalarTy, matData, K, "gMatImmediate");
...
static GlobalVariable* createPackedKindTable(
  Module &M,
  unsigned N,
  const std::vector<IRMatDataCUDA> &matData,
  StringRef name,
  unsigned addrSpace)
{
  ...
  std::vector<Constant*> init; init.reserve(N * N);
  ...
  auto *gv = new GlobalVariable(
    M, arrTy, /*isConst=*/true, GlobalValue::PrivateLinkage, CA,
    name, /*Before=*/nullptr, GlobalValue::NotThreadLocal, /*AS=*/addrSpace);
  gv->setAlignment(MaybeAlign(16));
  return gv;
}

// used to avoid recomputing the “pdep” style mapping in inner loops
static GlobalVariable* createDeltaLUT(
  Module &M,
  unsigned k,
  ...
  unsigned addrSpace)
{
  ...
  std::vector<Constant*> init; init.reserve(N);
  for (unsigned c = 0; c < N; ++c) {
    uint64_t delta = 0;
    for (unsigned b = 0; b < k; ++b)
        if (c & (1u << b)) delta |= (1ull << qubits[b]);
...
Value *buildOffset(
    ...
    */
    ...
    for (int q = 0; q <= highestQ; q++) {
        ...
        LLVM_DEBUG(std::cerr << " (globalThreadIdx & "
                             << utils::fmt_0b(mask, 32)
                             << ") << " << (qIdx - 1) << "\n";);
        mask = 0ULL;
    }
  ...
  Type *scalarTy)
{
  ...
  // Fast-math
  FastMathFlags FMF;
  FMF.setNoNaNs(); FMF.setNoInfs();
  FMF.setNoSignedZeros(); FMF.setAllowContract(true); FMF.setAllowReassoc(true);
  ...
  // for (unsigned i=0;i<N*N;++i) {
  //     kinds.push_back(ConstantInt::get(B.getInt8Ty(), mapKind(matData[i].reKind)));
  //     kinds.push_back(ConstantInt::get(B.getInt8Ty(), mapKind(matData[i].imKind)));
  // }
  ...
  if (!config.assumeContiguousTargets) {
      const size_t deltaBytes = size_t(N) * sizeof(uint64_t);
      const unsigned AS_DELTA = (deltaBytes <= 64 * 1024) ? 4 : 1;
      gDelta = createDeltaLUT(M, k, qubits, (Twine("gDelta_") + func->getName()).str(), AS_DELTA);
  ...
  Value *ntid32 = B.CreateIntrinsic(B.getInt32Ty(), Intrinsic::nvvm_read_ptx_sreg_ntid_x, {});
  ...
  Value *r0 = B.CreateAdd(
      B.CreateZExt(lane32, B.getInt64Ty()),
      B.CreateShl(B.CreateZExt(warpInBlock, B.getInt64Ty()), 5), "r0");
  Value *rStep = B.CreateShl(B.CreateZExt(warpsPerBlck, B.getInt64Ty()),
                              5, "r.step");
  ...
  Value *gridX = B.CreateIntrinsic(B.getInt32Ty(), Intrinsic::nvvm_read_ptx_sreg_nctaid_x, {});

  // comboSlot = ctaid.x / tilesPerGate
  // comboStride = gridDim.x / tilesPerGate (integral; launcher guarantees)
  Value *comboSlot = B.CreateUDiv(bid32, B.getInt32(tilesPerGate), "combo.slot");
  Value *comboStride = B.CreateUDiv(gridX, B.getInt32(tilesPerGate), "combo.stride");

  // Fetch p.combos (uint32 arg named exactly "p.combos")
  Argument *combosV = nullptr;
  for (auto &A : func->args()) { if (A.getName() == "p.combos") { combosV = &A; break; } }
  ...
  PHINode *accIm = B.CreatePHI(scalarTy, 2, "accIm");  // Σ (matRe*xIm + matIm*xRe)
  ...
  B.SetInsertPoint(cBody);
  {
      ...
      // Load kinds
      auto toI32 = [&](Value *v) { return B.CreateIntCast(v, B.getInt32Ty(), false); };
      Value *lin32 = toI32(lin);
      Value *kByte = B.CreateLoad(B.getInt8Ty(),
      ...
      Value *kIm = B.CreateAnd(B.CreateLShr(kByte32, B.getInt32(4)), B.getInt32(0xF));
      ...
      Value *accImn = B.CreateFAdd(accIm,
                              B.CreateFAdd(B.CreateFMul(mRe, xIm),
  ...
  Type *scalarTy)
{
  ...
  for (const auto &md : matData) {
      assert(md.reKind != SK_Runtime && md.imKind != SK_Runtime &&
  ...
  for (auto &A : func->args()) {
      if (A.getName() == "p.combos") { combosV = &A; break; }
  }
  assert(combosV && "Missing kernel argument 'p.combos' (uint32)");
  ...
  GlobalVariable *gMatImm =
      createGlobalMatrixArray_SharedTiledImm(M, scalarTy, N, matData,
                                              "gMatImmSharedTiled", AS_M);
  auto *arrTyM = llvm::cast<ArrayType>(gMatImm->getValueType());
  Value *gMatBase =
      B.CreateInBoundsGEP(arrTyM, gMatImm, {B.getInt32(0), B.getInt64(0)}, "gMat.base");
  ...
  // for (unsigned i = 0; i < N * N; ++i) {
  //     kindInit.push_back(ConstantInt::get(B.getInt8Ty(), mapKind(matData[i].reKind)));
  //     kindInit.push_back(ConstantInt::get(B.getInt8Ty(), mapKind(matData[i].imKind)));
  // }
  ...
  Value *tid32 = B.CreateIntrinsic(B.getInt32Ty(), Intrinsic::nvvm_read_ptx_sreg_tid_x, {});
  Value *bid32 = B.CreateIntrinsic(B.getInt32Ty(), Intrinsic::nvvm_read_ptx_sreg_ctaid_x, {});
  Value *gridX = B.CreateIntrinsic(B.getInt32Ty(), Intrinsic::nvvm_read_ptx_sreg_nctaid_x, {}); // gridDim.x

  // Only the first TILE threads per CTA act within a tile
  Value *tid32L = (TILE == 1) ? tid32 : B.CreateURem(tid32, B.getInt32(TILE), "tid.in.tile");
  ...
  Value *rowBase64 = B.CreateMul(B.CreateZExt(rowTileIdx32, B.getInt64Ty()),
                                      B.getInt64(TILE), "row.base");
  ...
  // Persistent-grid combos loop:
  // comboSlot = ctaid.x / tilesPerGate
  // comboStride = gridDim.x / tilesPerGate (assume divisible; launcher enforces)
...
Value *accImTile = accImLC;
  ...
  for (unsigned t = 0; t < TILE; ++t) {
    ...
    B.SetInsertPoint(yesBB);
    {
        ...
        // auto toI32 = [&](Value *v64) { return B.CreateIntCast(v64, i32Ty, false); };
        // Value *kRePtr = B.CreateGEP(kindArrTy, gMatKind, {B.getInt32(0), toI32(idx2)});
        // Value *kImPtr = B.CreateGEP(kindArrTy, gMatKind,
        //                             {B.getInt32(0), B.CreateAdd(toI32(idx2), B.getInt32(1))});
  ...
  // (4) Store result (only tid < TILE); use loop-carried sums which dominate exit
  ...
  B.SetInsertPoint(stYes);
  {
    ...
    Value *dstPairPtr = bitCastPtrToVec2(B, B.CreateGEP(scalarTy, svBaseCombo, off2), scalarTy);
...
void genMatrixVectorMultiplyFromPointer_SharedTiled(
    ...
    ArrayType *smVecTy = ArrayType::get(scalarTy, 2 * TILE);
    auto *smVecGV = new GlobalVariable(
        M, smVecTy, /*constant=*/false, GlobalValue::PrivateLinkage, UndefValue::get(smVecTy),
        "TileX", nullptr, GlobalValue::NotThreadLocal, /*AddressSpace=*/3
    );
    smVecGV->setAlignment(MaybeAlign(8));
    Value *smVecBase = B.CreateGEP(smVecTy, smVecGV, {B.getInt32(0), B.getInt32(0)});
    ...
    Value *row64 = B.CreateAdd(rowBase64, B.CreateZExt(tid32, B.getInt64Ty()), "row64");
    ...
    /* 1. Load TILE amplitudes of |x〉 into shared memory */
    {
        ...
        B.SetInsertPoint(ldYes);
        {
            ...
            B.CreateStore(B.CreateLoad(scalarTy, srcRe), B.CreateGEP(scalarTy, smVecBase, idxS));
        ...
        B.SetInsertPoint(ldNo);
        {
            ...
            Value *idxS = B.CreateMul(B.CreateZExt(tid32, B.getInt64Ty()), B.getInt64(2));
    ...
    for (unsigned t = 0; t < TILE; ++t) {
        ...
        B.SetInsertPoint(yesBB);
        {
            ...
            Value *mRe = B.CreateLoad(scalarTy, B.CreateGEP(scalarTy, matBasePtr, idx2));
            ...
            Value *xRe = B.CreateLoad(scalarTy, B.CreateGEP(scalarTy, smVecBase, offX));
...
static void genMatrixVectorMultiply_InlineImm(
    IRBuilder<> &B,
    const CUDAKernelGenConfig &config,
    const ComplexSquareMatrix &/*matrix*/,
    const QuantumGate::TargetQubitsType &qubits,
    ...
    for (unsigned r = 0; r < K; ++r) {
        ...
        Value *accIm  = ConstantFP::get(scalarTy, 0.0); // Σ matRe*oldIm + matIm*oldRe

        for (unsigned c = 0; c < K; ++c) {
            const auto &md = matData[r * K + c];
            if (md.reKind == SK_Zero && md.imKind == SK_Zero) continue;
    ...
    Type *scalarTy)
{
    ...
    Value *tid32 = B.CreateIntrinsic(B.getInt32Ty(), Intrinsic::nvvm_read_ptx_sreg_tid_x, {});
    Value *ntid32 = B.CreateIntrinsic(B.getInt32Ty(), Intrinsic::nvvm_read_ptx_sreg_ntid_x, {});
    Value *cta = B.CreateIntrinsic(B.getInt32Ty(), Intrinsic::nvvm_read_ptx_sreg_ctaid_x, {});
    Value *gridX = B.CreateIntrinsic(B.getInt32Ty(), Intrinsic::nvvm_read_ptx_sreg_nctaid_x, {});
    ...
    Value *start32 = B.CreateAdd(
        B.CreateMul(B.CreateAdd(B.CreateMul(cta, warpsPerCTA), warpIn), B.getInt32(32)),
        lane, "combo.start");

    // stride = gridDim.x * warpsPerCTA * 32
    Value *stride32 = B.CreateMul(B.CreateMul(gridX, warpsPerCTA), B.getInt32(32), "combo.stride");
    ...
    B.SetInsertPoint(cmbBody);
    {
        ...
        genMatrixVectorMultiply_InlineImm(B, cfg, matrix, qLSB, matData, svComboBase, scalarTy);
...
void genMatrixVectorMultiplyFromPointer(
    ...
    B.SetInsertPoint(loadBodyBB);
    {
        ...
        Value *rePtr = B.CreateGEP(scalarTy, svPtrV, twoDelta, "rePtr");
        Value *imPtr = B.CreateGEP(
            scalarTy, svPtrV, B.CreateAdd(twoDelta, ConstantInt::get(B.getInt64Ty(), 1)), "imPtr"
        );
    ...
    B.SetInsertPoint(innerBodyBB);
    {
        ...
        Value *imPtr = B.CreateGEP(
            scalarTy, matBasePtr, B.CreateAdd(base2, ConstantInt::get(B.getInt64Ty(), 1)), "matImPtr"
        );
        Value *matRe = B.CreateLoad(scalarTy, rePtr, "matRe");
        Value *matIm = B.CreateLoad(scalarTy, imPtr, "matIm");
    ...
    Value *outRePtr = B.CreateGEP(scalarTy, svPtrV, twoDelta);
    Value *outImPtr = B.CreateGEP(
        scalarTy, svPtrV, B.CreateAdd(twoDelta, ConstantInt::get(B.getInt64Ty(), 1))
    );
    B.CreateStore(newReAmp, outRePtr);
    B.CreateStore(newImAmp, outImPtr);
...
void genMatrixVectorMultiplyFromConst(
    ...
    for (unsigned r = 0; r < K; ++r) {
        ...
        Value *accIm = ConstantFP::get(scalarTy, 0.0); // sum(matRe*oldIm + matIm*oldRe)

        for (unsigned c = 0; c < K; ++c) {
            const IRMatDataCUDA &md = matData[r * K + c];

            // Skip entries that are exactly zero (within tolerance)
            if (md.reKind == SK_Zero && md.imKind == SK_Zero) continue; // no IR for this (r,c)
            ...
            if (md.reKind == SK_Runtime || md.imKind == SK_Runtime) {
                ...
                Value *baseScalarPtr = B.CreateGEP(
                    arrTy, gConstMat, {B.getInt32(0), idx2}
                );
...
GlobalVariable *getOrCreateConstMatGlobal(
    Module &M,
    Type *arrTy,
    StringRef globalName
) {
    // Check if already exists
    if (auto *gv = M.getNamedGlobal(globalName)) return gv;
...
//     // if (cfg.matrixLoadMode == CUDAMatrixLoadMode::UseMatImmValues && k <= 3)
...
//     Type *scalarTy = (cfg.precision == Precision::F32) ? B.getFloatTy() : B.getDoubleTy();
...
//             // Compute combosIdx = ctaid.x / tilesPerGate (tilesPerGate is compile-time const here)
//             const unsigned N = 1u << k;
//             const unsigned TILE = std::min(256u, N);
//             const unsigned tilesPerGate = (N + TILE - 1u) / TILE;

//             auto *cta = B.CreateIntrinsic(B.getInt32Ty(), Intrinsic::nvvm_read_ptx_sreg_ctaid_x, {});
//             auto *combosIdx32 = B.CreateUDiv(cta, B.getInt32(tilesPerGate));
//             auto *combosIdx64 = B.CreateZExt(combosIdx32, B.getInt64Ty());
...
//         //     // // heuristic: inline up to K<=16 (k<=4) for f32, K<=8 (k<=3) for f64
//         //     // const bool inlineOK =
//         //     //     (cfg.precision == Precision::F32) ? (k <= 4) : (k <= 3);
//         //     // if (inlineOK) {
//         //     //     genMatrixVectorMultiply_InlineImm(B, cfg, matrix, qubits, matData, svPtrV, scalarTy);
//         //     //     func->addFnAttr("cast.kstyle", "imm-inline");
//         //     // } else {
//         //     //     genMatrixVectorMultiply_SharedTiled(B, cfg, matrix, qubits, matData, svPtrV, scalarTy);
...
//         //     const bool inlineOK = (cfg.precision == Precision::F32) ? (k <= 4) : (k <= 3);
//         //     if (inlineOK) {
//         //       genMatrixVectorMultiply_ImmRowPerLane(B, cfg, qLSB, matData, svPtrV, scalarTy);
//         //       // genMatrixVectorMultiply_InlineImm(B, cfg, matrix, qLSB, matData, svPtrV, scalarTy);
//         //       // func->addFnAttr("cast.kstyle", "imm-inline");
//         //     } else {
//         //         genMatrixVectorMultiply_SharedTiled(B, cfg, matrix, qLSB, matData, svPtrV, scalarTy);
//         //         func->addFnAttr("cast.kstyle", "imm-shared");
//         //         // genMatrixVectorMultiply_SharedTiled_WarpPerRow(B, cfg, matrix, qLSB, matData, svPtrV, scalarTy);
//         //     }
//         //     break;
//         // }
...
//             }
//             // Heuristic: warp-per-row for small/medium K; shared-tiled for bigger K
//             // const bool useWPR = (cfg.precision == Precision::F32) ? (k <= 6) : (k <= 5);
//             // if (useWPR) {
//             //     genMatrixVectorMultiply_ImmRowPerLane(B, cfg, qLSB, matData, svPtrV, scalarTy);
//             //     // The callee sets cast.kstyle="imm-shared-warp" + cast.warps
//             // } else {
//             //     genMatrixVectorMultiply_SharedTiled(B, cfg, matrix, qLSB, matData, svPtrV, scalarTy);
//             //     func->addFnAttr("cast.kstyle", "imm-shared");
//             // }
...
//                 // Tiny gates → straight-line inline path (best warp efficiency and least CF)
//                 genMatrixVectorMultiply_InlineImm(B, cfg, matrix, qLSB, matData, svPtrV, scalarTy);
//                 func->addFnAttr("cast.kstyle", "imm-inline");
//             } else if (N >= 32 && N <= 64) {
...
//                 B, config, matrix, qubits, gConstMat, svPtrV, args.pMatArg, scalarTy, matData
//             );
...
CUDAKernelManager::genCUDAGateVariants_(
  const CUDAKernelGenConfig &config,
  ConstQuantumGatePtr gate,
  const std::string &baseName
) {
  auto buildOne = [&](bool assumeContiguous, const std::string &name) -> llvm::Function * {
      CUDAKernelGenConfig cfg = config;
      ...
      if (auto *stdQuGate = llvm::dyn_cast<const StandardQuantumGate>(gate.get());
          ...
          for (auto q : qs) both.push_back(q + nQ);
      ...
      if (fn) fn->addFnAttr("cast.addr", assumeContiguous ? "lsb" : "generic");
      return fn;
  };
...
Function *CUDAKernelManager::gen_(
  ...
  // Create the kernel function and plumb the arguments (p.sv, [p.mat], p.combos)
  IRArgsCUDA args;
  auto *func = getFunctionDeclarationCUDA(
  ...
  auto comboBasePtr = [&](unsigned tilesPerGate,
                          const QuantumGate::TargetQubitsType &qsForOffset) -> Value* {
      // ctaid.x
      auto *cta   = B.CreateIntrinsic(B.getInt32Ty(), Intrinsic::nvvm_read_ptx_sreg_ctaid_x, {});
      Value *idx32 = (tilesPerGate == 1) ? cta : B.CreateUDiv(cta, B.getInt32(tilesPerGate));
  ...
  switch (cfg.matrixLoadMode) {
      case CUDAMatrixLoadMode::UseMatImmValues: {
          ...
          // const bool inlineOK = (cfg.precision == Precision::F32) ? (N <= 8) : (N <= 4);
          // if (inlineOK) {
          ...
          //     // auto *cta   = B.CreateIntrinsic(B.getInt32Ty(), Intrinsic::nvvm_read_ptx_sreg_ctaid_x,   {});
          //     // auto *gridX = B.CreateIntrinsic(B.getInt32Ty(), Intrinsic::nvvm_read_ptx_sreg_nctaid_x, {});

          //     // // combo loop scaffolding
          //     // auto *cmbChk  = BasicBlock::Create(B.getContext(), "cmb.chk",  func);
          //     // auto *cmbBody = BasicBlock::Create(B.getContext(), "cmb.body", func);
          //     // auto *cmbInc  = BasicBlock::Create(B.getContext(), "cmb.inc",  func);
          //     // auto *cmbDone = BasicBlock::Create(B.getContext(), "cmb.done", func);
          ...
          //     //                 : B.CreateShl(buildOffset(B, idx64, qLSB), 1);
          //     //   Value *svComboBase = B.CreateGEP(scalarTy, args.pSvArg, base2, "sv.combo.base");
          //     //   genMatrixVectorMultiply_InlineImm(B, cfg, matrix, qLSB, matData, svComboBase, scalarTy);
          //     //   B.CreateBr(cmbInc);
          //     // }
          ...
          // // Medium (one warp handles a row each): the kernel loops over combos internally
          // if (N >= 32 && N <= 64) {
          //     // Pass the full SV base; the callee reads p.combos and iterates combos
      ...
      case CUDAMatrixLoadMode::LoadInDefaultMemSpace: {
          ...
          const unsigned N = 1u << k;
          const unsigned TILE = std::min(
              (cfg.precision == Precision::F64) ? 128u : 256u, N
          );
      ...
      case CUDAMatrixLoadMode::LoadInConstMemSpace: {
          // Build/lookup constant-memory global (AS=4) and initialize with matrix values
          auto *arrTy = ArrayType::get(scalarTy, 2u * KK);
          auto *gConstMat = getOrCreateConstMatGlobal(
              *llvmContextModulePair.llvmModule, arrTy, "gConstMatShared"
          );
          ...
          Value *svComboBase = comboBasePtr(/*tilesPerGate=*/1, qLSB);
          genMatrixVectorMultiplyFromConst(
              B, cfg, matrix, qLSB, gConstMat, svComboBase, args.pMatArg, scalarTy, matData
          );
          func->addFnAttr("cast.kstyle", "const-small");
          break;
      }
...
CUDAKernelManager::getOrBuildKernel_(
    ...
    const unsigned k = qubits.size();
    const bool imm = (baseCfg.matrixLoadMode == CUDAMatrixLoadMode::UseMatImmValues);
    const uint64_t matHash = imm ? hashMatrixImm(M, 1u << k, baseCfg.precision == Precision::F32)
                                 : 0ull;

    KernelKey key{ k, baseCfg.precision, baseCfg.matrixLoadMode, assumeContiguous, matHash };
...
CUDAKernelManager::genCUDAGate_(
    const CUDAKernelGenConfig &config,
    ConstQuantumGatePtr gate,
    const std::string &funcName
) {
    ...
    //     ki->oneThreadPerBlock = (style == "imm-inline" || style == "const-small");
    // } else {
    //     ki->oneThreadPerBlock = false;
    // }
...
MaybeError<void>
CUDAKernelManager::genStandaloneGate(
    const CUDAKernelGenConfig &config,
    ConstQuantumGatePtr gate,
    const std::string &_funcName
) {
    std::string funcName(_funcName);
    if (funcName.empty()) funcName = "kernel_" + std::to_string(standaloneKernels_.size());

    // check for name conflicts
    for (const auto &kernel : standaloneKernels_) {
        if (kernel->llvmFuncName == funcName) {
            return cast::makeError<void>("Kernel with name '" + funcName + "' already exists.");
        }
    }
...
MaybeError<void>
CUDAKernelManager::genGraphGates(
    const CUDAKernelGenConfig &config,
    ...
    for (const auto &gate : allGates) {
        ...
        const std::string base =
            mangledGraphName + "_" + std::to_string(order++) + "_" + std::to_string(graph.gateId(gate));
...
void CUDAKernelManager::rebuildOrderedKernelIndex_() {
  ...
  auto findByName = [&](const std::string &name) -> CUDAKernelInfo * {
      for (auto &k : standaloneKernels_) {
          if (k->llvmFuncName == name) return k.get();
      }
