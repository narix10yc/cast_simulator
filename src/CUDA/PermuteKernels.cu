...
template<typename T>
__global__ void swap_two_axes(T* __restrict svReIm,
  ...
  for (uint64_t i = blockIdx.x * blockDim.x + threadIdx.x;
       i < dim; i += (uint64_t)gridDim.x * blockDim.x) {
...
void applyPermutationOnGPU(void* svReIm,
  ...
  for (auto [a,b] : swaps) {
    if (sizeofScalar == 4)
      swap_two_axes<float><<<blocks, TB, 0, stream>>>((float*)svReIm, nQubits, a, b);
    else
      swap_two_axes<double><<<blocks, TB, 0, stream>>>((double*)svReIm, nQubits, a, b);
  }
}
} // namespace cast
