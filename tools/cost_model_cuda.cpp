...
cl::opt<int>
ArgWorkerThreads("workerThreads", cl::desc("Number of CPU worker threads for JIT compilation"), cl::Required);
...
int main(int argc, char** argv) {
  ...
  if (inFile.peek() == std::ifstream::traits_type::eof())
    outFile << "nQubits,opCount,precision,blockSize,occupancy,coalescing,memSpd\n";
  inFile.close();

  CUDAPerformanceCache cache;
  CUDAKernelGenConfig cudaConfig;
  cudaConfig.blockSize = ArgBlockSize;
  cache.runExperiments(cudaConfig, ArgNQubits, ArgBlockSize, ArgNTests, ArgWorkerThreads);
