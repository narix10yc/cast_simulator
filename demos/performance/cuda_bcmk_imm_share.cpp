...
int main() {
  ...
  // Build the real legacy::CircuitGraph
  ...
  TimingResult kernelGenTR = kernelGenTimer.timeit([&]() {
    ...
    localKM.genCUDAGatesFromlegacy::CircuitGraphMulti(
  ...
  // kernelMgr.genCUDAGatesFromlegacy::CircuitGraph(genConfig, *graphPtr,
  // "testCircuit");
  kernelMgr.genCUDAGatesFromlegacy::CircuitGraphMulti(
      genConfig, *graphPtr, "testCircuit");
  ...
  TimingResult execTR = execTimer.timeit([&]() {
    ...
    auto& allKernels =
        kernelMgr.kernels(); // returns std::vector<CUDAKernelInfo>

    // Launch each kernel in the manager
    for (auto& kInfo : allKernels) {
      kernelMgr.launchCUDAKernel(sv.dData(),   // device pointer to statevector
