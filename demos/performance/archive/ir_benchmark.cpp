...
int main(int argc, char** argv) {
  ...
  for (unsigned i = 0; i < nQubitsVec.size(); i++) {
    ...
    rst = timer.timeit([&]() {
      applySingleQubitQuEST<real_t>(
          sv.real, sv.imag, _metaData[0].mPtr, nQubits, Q0_VALUE);
    });
    // rst.display();
    tVec[i] = rst.min;
  }
  ...
  for (unsigned i = 0; i < nQubitsVec.size(); i++) {
    ...
    const uint64_t idxMax = 1ULL << (nQubits - 4);
    rst = timer.timeit([&]() {
      applySingleQubitTemplate<real_t, Q0_VALUE>(
          sv.real, sv.imag, _metaData[0].mPtr, nQubits);
    });
    // rst.display();
    tVec[i] = rst.min;
  }
