...
static void printTimingStats(const std::string& label,
  ...
  std::vector<double> s = tr.tArr;
  std::sort(s.begin(), s.end());
  double med = s.size() % 2 ? s[s.size() / 2]
                            : 0.5 * (s[s.size() / 2 - 1] + s[s.size() / 2]);

  cout << std::fixed << std::setprecision(3) << label
       << ": mean=" << (mean * 1e3) << " ms ± " << (stdev * 1e3)
...
static inline void ltrim(std::string& s) {
  size_t i = 0;
  while (i < s.size() && std::isspace(unsigned(s[i])))
    ++i;
  if (i)
    s.erase(0, i);
}
...
static Precision parsePrecisionToken(std::string t) {
  t = lower(trim(t));
  if (t == "f32" || t == "32" || t == "float" || t == "single")
    return Precision::F32;
  if (t == "f64" || t == "64" || t == "double")
    return Precision::F64;
  // numeric enum fallback
  try {
    int v = std::stoi(t);
    if (v == int(Precision::F32))
      return Precision::F32;
    if (v == int(Precision::F64))
      return Precision::F64;
  } catch (...) {
  }
...
loadGpuPerfCSV(const std::string& path) {
  ...
  if (line != header) {
    ...
    if (toks.size() >= 20) {
      ...
      int gx = std::stoi(toks[i++]), gy = std::stoi(toks[i++]),
          gz = std::stoi(toks[i++]);
      int bx = std::stoi(toks[i++]), by = std::stoi(toks[i++]),
          bz = std::stoi(toks[i++]);
...
static Args parseCLI(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    auto next = [&](int& i) -> std::string {
      if (i + 1 >= argc) {
        std::cerr << "Missing value after " << v << "\n";
        std::exit(1);
      }
    ...
    if (v == "-i" || v == "--input")
      ...
      a.reps = std::stoi(next(i));
    else if (v == "--block")
      a.blockSize = std::stoi(next(i));
    else if (v == "--threads")
      a.nThreads = std::stoi(next(i));
    else if (v == "--prec")
      a.precBits = std::stoi(next(i)); // 32/64
    else if (v == "--no-fusion")
      a.fuse = false;
    else if (v == "--perf-csv")
      a.perfCSV = next(i); // GPU CSV
    else if (v == "--opt") {
      std::string s = next(i);
      if (s == "O0")
        a.opt = llvm::OptimizationLevel::O0;
      else if (s == "O1")
        a.opt = llvm::OptimizationLevel::O1;
      else if (s == "O2")
        a.opt = llvm::OptimizationLevel::O2;
      else if (s == "O3")
        a.opt = llvm::OptimizationLevel::O3;
      else {
        std::cerr << "Unknown --opt " << s << "\n";
        std::exit(1);
      }
    } else {
...
int main(int argc, char** argv) {
  ...
  std::cout << "[Info] QASM: " << args.qasmFile << ", prec=" << args.precBits
            << ", block=" << args.blockSize << ", reps=" << args.reps
  ...
  if (args.fuse) {
    ...
    const Precision prec =
        (args.precBits == 32) ? Precision::F32 : Precision::F64;
    ...
    if (!args.perfCSV.empty()) {
      try {
        ...
        auto cfg = std::make_unique<CUDAFusionConfig>(
            std::move(model), args.blockSize, prec);
        cfg->sizeMin = 2;
        cfg->sizeMax = 5;
  ...
  genCfg.precision =
      (args.precBits == 32 ? cast::Precision::F32 : cast::Precision::F64);
  ...
  std::vector<const CUDAKernelInfo*> kernels;
  {
    ...
    for (const auto& g : gates) {
      const std::string base = prefix + "_" + std::to_string(order++) + "_" +
                               std::to_string(graph->gateId(g));

      const CUDAKernelInfo* ki = km.getKernelByName(base + "_lsb");
      if (!ki)
        ki = km.getKernelByName(base + "_gen");
