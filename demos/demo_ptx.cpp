...
cl::opt<bool> ArgRunNaiveFuse("run-naive-fuse",
                              cl::desc("Run naive-fuse circuit"),
                              cl::init(true));
...
cl::opt<int> ArgBlockSize(
    "blocksize",
    cl::desc(
        "Block size used in CUDA kernels (min 32, max 512, in powers of 2)"),
    cl::init(64));
...
cl::opt<std::string> ArgCUDAModelPath(
    "cuda-model", cl::desc("Path to CUDA performance model"), cl::init(""));

cl::opt<bool> ArgRunCudaFuse("run-cuda-fuse",
                             cl::desc("Run CUDA-optimized fusion"),
                             cl::init(false));
...
int main(int argc, const char** argv) {
  ...
  // This is temporary work-around as legacy::CircuitGraph does not allow copy
  // yet
  ...
  if (ArgRunAdaptiveFuse && ArgModelPath != "") {
    ...
    applyGateFusion(
        fusionConfigAggresive, &standardCostModel, graphAdaptiveFuse);
  }
  ...
  // JIT compile kernels
  std::vector<CUDAKernelInfo*> kernelsNoFuse, kernelsNaiveFuse,
      kernelAdaptiveFuse, kernelCudaFuse;
  utils::timedExecute(
      [&]() {
        ...
        if (ArgRunNoFuse) {
          opCountTotal = 0.0;
          kernelsNoFuse = kernelMgr.collectCUDAKernelsFromlegacy::CircuitGraph(
              "graphNoFuse");
        ...
        if (ArgRunNaiveFuse) {
          opCountTotal = 0.0;
          kernelsNaiveFuse =
              kernelMgr.collectCUDAKernelsFromlegacy::CircuitGraph(
                  "graphNaiveFuse");
        ...
        if (ArgRunCudaFuse && ArgCUDAModelPath != "") {
          opCountTotal = 0.0;
          kernelCudaFuse = kernelMgr.collectCUDAKernelsFromlegacy::CircuitGraph(
              "graphCudaFuse");
