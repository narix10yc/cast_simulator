...
public:
  ...
  Token(TokenKind kind = tk_Unknown) : kind(kind), loc(nullptr, nullptr) {}
  ...
  double toDouble() const { return std::stod(std::string(loc.begin, loc.end)); }
  ...
  std::string toString() const { return std::string(loc.begin, loc.end); }
