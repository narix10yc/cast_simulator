...
inline Layout makeIdentityLayout(unsigned n) {
  Layout L;
  L.physPosOfLog.resize(n);
  L.logOfPhys.resize(n);
...
inline std::vector<AxisSwap>
planBringTargetsToLSB(Layout L, std::span<const int> targets) {
  const unsigned k = targets.size();
...
inline bool
subsetFitsLSB(const Layout& L, std::span<const int> targets, unsigned window) {
  for (int q : targets)
    if (L.physPosOfLog[q] >= (int)window)
      return false;
  return true;
}
