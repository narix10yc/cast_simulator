...
public:
  ...
  llvm::Function* generateKernel(const cast::legacy::QuantumGate& gate,
                                 const std::string& funcName = "") {
    return generateKernelDebug(gate, 0, funcName);
  }

  llvm::Function* generateKernelDebug(const cast::legacy::QuantumGate& gate,
                                      int debugLevel,
                                      const std::string& funcName = "");

  llvm::Function* generateCUDAKernel(const cast::legacy::QuantumGate& gate,
