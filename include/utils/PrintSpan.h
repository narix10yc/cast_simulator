...
concept Printer_C =
    requires(Printer_T printer, std::ostream& os, const T& value) {
...
std::ostream&
printSpanWithPrinterNoBracket(std::ostream& os, std::span<T> v, Printer_T f) {
  auto it = v.begin();
  auto e = v.end();
  if (it == e)
    return os;
...
template <typename T, typename Printer_T>
  requires Printer_C<Printer_T, T>
std::ostream&
printSpanWithPrinter(std::ostream& os, std::span<T> v, Printer_T f) {
  os.put('[');
