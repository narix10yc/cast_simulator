...
static double calculateMemSpd(int nQubits, double time) {
  return (static_cast<double>(1ULL << nQubits) * sizeof(ScalarType) * 2) /
         (time * 1e9);
}
...
int main() {
  ...
  config.matrixLoadMode = CPUMatrixLoadMode::StackLoadMatElems;
  config.zeroTol = 0.0;
  config.oneTol = 0.0;

  CPUKernelManager kernelMgr;
  for (int k = 0; k < NQUBITS; ++k) {
    if (!kernelMgr.genStandaloneGate(config,
  ...
  std::size_t offset =
      misalignment == 0 ? 0 : (64 - misalignment) / sizeof(double);
