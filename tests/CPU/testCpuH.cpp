#include "cast/CPU/CPUKernelManager.h"
#include "cast/CPU/CPUStatevector.h"
#include "tests/TestKit.h"

using namespace cast;
using namespace utils;

static QuantumGatePtr getH(int q) {
  return StandardQuantumGate::Create(ScalarGateMatrix::H(), nullptr, {q});
}

template <CPUSimdWidth SimdWidth> static bool f() {
  test::TestSuite suite(
      "CPU H-kernel behavior (SIMD=" + std::to_string(SimdWidth) + ")");

  CPUKernelManager km;

  CPUKernelGenConfig genCfg(SimdWidth, Precision::FP64);

  llvm::cantFail(km.genGate(genCfg, getH(0), "gate_h_0"));
  llvm::cantFail(km.genGate(genCfg, getH(1), "gate_h_1"));
  llvm::cantFail(km.genGate(genCfg, getH(2), "gate_h_2"));
  llvm::cantFail(km.genGate(genCfg, getH(3), "gate_h_3"));

  llvm::cantFail(km.compileAllPools());

  CPUStatevector<double> sv(6, SimdWidth);
  sv.initialize();
  suite.assertCloseFP64(
      sv.norm(), 1.0, "Initialized state has unit norm", GET_INFO());
  suite.assertCloseFP64(sv.prob(0),
                        0.0,
                        "Initialized state qubit-0 probability is zero",
                        GET_INFO());

  llvm::cantFail(km.applyCPUKernel(sv.data(), sv.nQubits(), "gate_h_0"));
  suite.assertCloseFP64(
      sv.norm(), 1.0, "H on qubit 0 preserves norm", GET_INFO());
  suite.assertCloseFP64(sv.prob(0),
                        0.5,
                        "H on qubit 0 sets qubit-0 probability to 0.5",
                        GET_INFO());

  sv.initialize();
  llvm::cantFail(km.applyCPUKernel(sv.data(), sv.nQubits(), "gate_h_1"));
  suite.assertCloseFP64(
      sv.norm(), 1.0, "H on qubit 1 preserves norm", GET_INFO());
  suite.assertCloseFP64(sv.prob(1),
                        0.5,
                        "H on qubit 1 sets qubit-1 probability to 0.5",
                        GET_INFO());

  sv.initialize();
  llvm::cantFail(km.applyCPUKernel(sv.data(), sv.nQubits(), "gate_h_2"));
  suite.assertCloseFP64(
      sv.norm(), 1.0, "H on qubit 2 preserves norm", GET_INFO());
  suite.assertCloseFP64(sv.prob(2),
                        0.5,
                        "H on qubit 2 sets qubit-2 probability to 0.5",
                        GET_INFO());

  sv.initialize();
  llvm::cantFail(km.applyCPUKernel(sv.data(), sv.nQubits(), "gate_h_3"));
  suite.assertCloseFP64(
      sv.norm(), 1.0, "H on qubit 3 preserves norm", GET_INFO());
  suite.assertCloseFP64(sv.prob(3),
                        0.5,
                        "H on qubit 3 sets qubit-3 probability to 0.5",
                        GET_INFO());

  // randomized tests
  std::vector<double> pBefore(sv.nQubits()), pAfter(sv.nQubits());
  sv.randomize();
  suite.assertCloseFP64(
      sv.norm(), 1.0, "Randomized state has unit norm", GET_INFO());

  for (int q = 0; q < sv.nQubits(); q++)
    pBefore[q] = sv.prob(q);
  llvm::cantFail(km.applyCPUKernel(sv.data(), sv.nQubits(), "gate_h_0"));
  for (int q = 0; q < sv.nQubits(); q++)
    pAfter[q] = sv.prob(q);
  pAfter[0] = pBefore[0]; // probability could only change at the applied qubit
  suite.assertCloseFP64(
      sv.norm(), 1.0, "Random state: H on qubit 0 preserves norm", GET_INFO());
  suite.assertAllClose(
      pBefore,
      pAfter,
      "Random state: only target-qubit marginal may change (q=0)",
      GET_INFO());

  for (int q = 0; q < sv.nQubits(); q++)
    pBefore[q] = sv.prob(q);
  llvm::cantFail(km.applyCPUKernel(sv.data(), sv.nQubits(), "gate_h_1"));
  for (int q = 0; q < sv.nQubits(); q++)
    pAfter[q] = sv.prob(q);
  pAfter[1] = pBefore[1]; // probability could only change at the applied qubit
  suite.assertCloseFP64(
      sv.norm(), 1.0, "Random state: H on qubit 1 preserves norm", GET_INFO());
  suite.assertAllClose(
      pBefore,
      pAfter,
      "Random state: only target-qubit marginal may change (q=1)",
      GET_INFO());

  for (int q = 0; q < sv.nQubits(); q++)
    pBefore[q] = sv.prob(q);
  llvm::cantFail(km.applyCPUKernel(sv.data(), sv.nQubits(), "gate_h_2"));
  for (int q = 0; q < sv.nQubits(); q++)
    pAfter[q] = sv.prob(q);
  pAfter[2] = pBefore[2]; // probability could only change at the applied qubit
  suite.assertCloseFP64(
      sv.norm(), 1.0, "Random state: H on qubit 2 preserves norm", GET_INFO());
  suite.assertAllClose(
      pBefore,
      pAfter,
      "Random state: only target-qubit marginal may change (q=2)",
      GET_INFO());

  for (int q = 0; q < sv.nQubits(); q++)
    pBefore[q] = sv.prob(q);
  llvm::cantFail(km.applyCPUKernel(sv.data(), sv.nQubits(), "gate_h_3"));
  for (int q = 0; q < sv.nQubits(); q++)
    pAfter[q] = sv.prob(q);
  pAfter[3] = pBefore[3]; // probability could only change at the applied qubit
  suite.assertCloseFP64(
      sv.norm(), 1.0, "Random state: H on qubit 3 preserves norm", GET_INFO());
  suite.assertAllClose(
      pBefore,
      pAfter,
      "Random state: only target-qubit marginal may change (q=3)",
      GET_INFO());

  return suite.displayResult();
}

bool test::test_cpuH() {
  const bool ok128 = f<W128>();
  const bool ok256 = f<W256>();
  return ok128 && ok256;
}
