...
static void f(const std::vector<int>& targetQubits) {
  test::TestSuite suite("Gate U" + std::to_string(targetQubits.size()) + "q (" + std::to_string(nQubits) + " qubits)");
  ...
  gates.emplace_back(std::make_shared<QuantumGate>(QuantumGate::RandomUnitary(targetQubits)));
  // gates.emplace_back(std::make_shared<QuantumGate>(QuantumGate::Identity(targetQubits)));
  ...
  for (size_t i = 0; i < gates.size(); i++) {
    kernelMgrCUDA.genCUDAGate(
      cudaGenConfig, gates[i], "gateImm_" + std::to_string(targetQubits.size()) + "q_" + std::to_string(i), nQubits);
  }
  ...
  for (size_t i = 0; i < gates.size(); i++) {
    ...
    kernelMgrCUDA.launchCUDAKernel(
        svCUDA0.dData(), svCUDA0.nQubits(), kernelMgrCUDA.kernels()[i], 1 << targetQubits.size());
